...
#[derive(Debug)]
pub enum CubicSolution<T: FloatingPoint> {
    None(T),
    All(T),
    OneReal(T),
    ...
    ThreeReal(T, T, T),
    TwoRealOneComplex(T, T, Complex<T>),
    OneRealTwoComplex(T, Complex<T>, Complex<T>),
}
pub trait CubicFormula<T: FloatingPoint>: QuadraticFormula {
    fn cubic_formula(a: Self, b: Self, c: Self, d: Self) -> CubicSolution<T>;
}

impl CubicFormula<f32> for f32 {
    fn cubic_formula(a: Self, b: Self, c: Self, d: Self) -> CubicSolution<f32> {
        ...
        if a == 0.0 {
            return match f32::quadratic_formula(b, c, d) {
                ...
                crate::equations::QuadraticSolution::TwoReal(r1, r2) => { CubicSolution::TwoReal(r1, r2) }
                crate::equations::QuadraticSolution::TwoComplex(c1, c2) => { CubicSolution::TwoComplex(c1, c2) }
            }
        }
